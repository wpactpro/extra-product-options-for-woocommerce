<?php
/**
 * This template displaying text field addon item.
 * This template can be overridden by copying it to yourtheme/eopfw-templates/epofw-text.php
...
$epofw_args         = array();
$field_type         = isset( $args['epofwtwp_args']['attr_data']['type'] ) ? $args['epofwtwp_args']['attr_data']['type'] : 'text';
...
if ( ! empty( $args['epofwtwp_args']['attr_data'] ) ) {
	foreach ( $args['epofwtwp_args']['attr_data'] as $attr_name => $attr_value ) {
		...
		if ( 'name' === $attr_name ) {
			...
			/**
			 * Apply filters for text name property.
			 *
			 * @since 2.5
			 */
			...
			/**
			 * Apply filters for text field property.
			 *
			 * @since 2.5
			 */
...
/**
 * Fire action before text addon field.
 *
 * @since 2.5
 */
...
/**
 * Fire action after text addon field.
 *
 * @since 2.5
 */
