<?php
/**
 * This template displaying timepicker field addon item.
 * This template can be overridden by copying it to yourtheme/eopfw-templates/epofw-timepicker.php
...
$epofw_args         = array();
$field_type         = isset( $args['epofwtwp_args']['attr_data']['type'] ) ? $args['epofwtwp_args']['attr_data']['type'] : 'timepicker';
...
if ( ! empty( $args['epofwtwp_args']['attr_data'] ) ) {
	foreach ( $args['epofwtwp_args']['attr_data'] as $attr_name => $attr_value ) {
		...
		if ( 'name' === $attr_name ) {
			...
			/**
			 * Apply filters for timepicker name field property.
			 *
			 * @since 2.5
			 */
			...
			/**
			 * Apply filters for timepicker type field property.
			 *
			 * @since 2.5
			 */
			$epofw_args[ $attr_name ] = apply_filters( 'epofw_field_property_' . $field_type . '_' . $attr_name, 'text', $args );
		} elseif ( 'class' === $attr_name ) {
			$attr_value = $attr_value . ' epofw_dft_' . $field_type;
			/**
			 * Apply filters for timepicker css field property.
			 *
			 * @since 2.5
			 */
			...
			/**
			 * Apply filters for timepicker field property.
			 *
			 * @since 2.5
			 */
			$epofw_args[ $attr_name ] = apply_filters( 'epofw_field_property_' . $field_type . '_' . $attr_name, $attr_value, $args );
...
/**
 * Fire action before timepicker addon field.
 *
 * @since 2.5
 */
...
/**
 * Fire action after timepicker addon field.
 *
 * @since 2.5
 */
