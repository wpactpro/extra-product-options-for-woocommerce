<?php
/**
 * This template displaying hidden addon item.
 * This template can be overridden by copying it to yourtheme/eopfw-templates/epofw-hidden.php
...
$epofw_args         = array();
$field_type         = isset( $args['epofwtwp_args']['attr_data']['type'] ) ? $args['epofwtwp_args']['attr_data']['type'] : 'hidden';
...
if ( ! empty( $args['epofwtwp_args']['attr_data'] ) ) {
	foreach ( $args['epofwtwp_args']['attr_data'] as $attr_name => $attr_value ) {
		...
		if ( 'name' === $attr_name ) {
			...
			/**
			 * Apply filters for hidden name field property.
			 *
			 * @since 2.5
			 */
			...
			/**
			 * Apply filters for hidden field property.
			 *
			 * @since 2.5
			 */
