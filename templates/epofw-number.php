<?php
/**
 * This template displaying number field addon item.
 * This template can be overridden by copying it to yourtheme/eopfw-templates/epofw-number.php
...
$epofw_args         = array();
$field_type         = isset( $args['epofwtwp_args']['attr_data']['type'] ) ? $args['epofwtwp_args']['attr_data']['type'] : 'number';
...
if ( ! empty( $args['epofwtwp_args']['attr_data'] ) ) {
	foreach ( $args['epofwtwp_args']['attr_data'] as $attr_name => $attr_value ) {
		...
		if ( 'name' === $attr_name ) {
			...
			/**
			 * Apply filters for number name field property.
			 *
			 * @since 2.5
			 */
			...
			/**
			 * Apply filters for number field property.
			 *
			 * @since 2.5
			 */
