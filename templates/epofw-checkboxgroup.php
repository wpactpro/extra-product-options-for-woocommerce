<?php
/**
 * This template displaying checkboxgroup addon item.
 * This template can be overridden by copying it to yourtheme/eopfw-templates/epofw-checkboxgroup.php
...
$epofw_args         = array();
$field_type         = isset( $args['epofwtwp_args']['attr_data']['type'] ) ? $args['epofwtwp_args']['attr_data']['type'] : 'checkboxgroup';
...
if ( ! empty( $args['epofwtwp_args']['attr_data'] ) ) {
	foreach ( $args['epofwtwp_args']['attr_data'] as $attr_name => $attr_value ) {
		...
		if ( 'name' === $attr_name ) {
			...
			/**
			 * Apply filters for checkboxgroup name field property.
			 *
			 * @since 2.5
			 */
			...
			/**
			 * Apply filters for checkboxgroup type field property.
			 *
			 * @since 2.5
			 */
			$epofw_args[ $attr_name ] = apply_filters( 'epofw_field_property_' . $field_type . '_' . $attr_name, 'checkbox', $args );
		} elseif ( 'class' === $attr_name ) {
			$attr_value = $attr_value . ' epofw_field_checkboxgroup';
			/**
			 * Apply filters for checkboxgroup class field property.
			 *
			 * @since 2.5
			 */
			...
			/**
			 * Apply filters for checkboxgroup field property.
			 *
			 * @since 2.5
			 */
			$epofw_args[ $attr_name ] = apply_filters( 'epofw_field_property_' . $field_type . '_' . $attr_name, $attr_value, $args );
...
if ( ! empty( $epofw_args ) && ! empty( $addon_options ) ) {
	foreach ( $addon_options as $opt_value ) {
		if ( ! empty( $opt_value ) ) {
			...
			/**
			 * Apply filters for checkboxgroup option field property.
			 *
			 * @since 2.5
			 */
