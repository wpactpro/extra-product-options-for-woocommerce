<?php
/**
 * This template displaying datepicker addon item.
 * This template can be overridden by copying it to yourtheme/eopfw-templates/epofw-datepicker.php
...
$epofw_args         = array();
$field_type         = isset( $args['epofwtwp_args']['attr_data']['type'] ) ? $args['epofwtwp_args']['attr_data']['type'] : 'datepicker';
...
if ( ! empty( $args['epofwtwp_args']['attr_data'] ) ) {
	foreach ( $args['epofwtwp_args']['attr_data'] as $attr_name => $attr_value ) {
		...
		if ( 'name' === $attr_name ) {
			...
			/**
			 * Apply filters for datepicker name field property.
			 *
			 * @since 2.5
			 */
			...
			/**
			 * Apply filters for datepicker type field property.
			 *
			 * @since 2.5
			 */
			...
			/**
			 * Apply filters for datepicker class field property.
			 *
			 * @since 2.5
			 */
			...
			/**
			 * Apply filters for datepicker field property.
			 *
			 * @since 2.5
			 */
			$epofw_args[ $attr_name ] = apply_filters( 'epofw_field_property_' . $field_type . '_' . $attr_name, $attr_value, $args );
