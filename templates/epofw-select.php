<?php
/**
 * This template displaying select field addon item.
 * This template can be overridden by copying it to yourtheme/eopfw-templates/epofw-select.php
...
$epofw_args = array();
$field_type = isset( $args['epofwtwp_args']['attr_data']['type'] ) ? $args['epofwtwp_args']['attr_data']['type'] : 'select';
if ( ! empty( $args['epofwtwp_args']['attr_data'] ) ) {
	foreach ( $args['epofwtwp_args']['attr_data'] as $attr_name => $attr_value ) {
		if ( 'name' === $attr_name ) {
			...
			/**
			 * Apply filters for select name field property.
			 *
			 * @since 2.5
			 */
			...
			/**
			 * Apply filters for select field property.
			 *
			 * @since 2.5
			 */
...
if ( ! empty( $epofw_args ) && ! empty( $addon_options ) ) {
	...
	<select
		...
		?>
	>
		<option value=""><?php esc_html_e( 'None', 'extra-product-options-for-woocommerce' ); ?></option>
		<?php
