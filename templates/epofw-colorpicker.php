<?php
/**
 * This template displaying colorpicker addon item.
 * This template can be overridden by copying it to yourtheme/eopfw-templates/epofw-colorpicker.php
...
$epofw_args         = array();
$field_type         = isset( $args['epofwtwp_args']['attr_data']['type'] ) ? $args['epofwtwp_args']['attr_data']['type'] : 'colorpicker';
...
if ( ! empty( $args['epofwtwp_args']['attr_data'] ) ) {
	foreach ( $args['epofwtwp_args']['attr_data'] as $attr_name => $attr_value ) {
		...
		if ( 'name' === $attr_name ) {
			...
			/**
			 * Apply filters for colorpicker name field property.
			 *
			 * @since 2.5
			 */
			...
			/**
			 * Apply filters for colorpicker type field property.
			 *
			 * @since 2.5
			 */
			...
			/**
			 * Apply filters for colorpicker class field property.
			 *
			 * @since 2.5
			 */
			...
			/**
			 * Apply filters for colorpicker field property.
			 *
			 * @since 2.5
			 */
			$epofw_args[ $attr_name ] = apply_filters( 'epofw_field_property_' . $field_type . '_' . $attr_name, $attr_value, $args );
